...
function CodeBlock({
  ...
  const isTerminal = label.toLowerCase().includes("terminal");
  ...
  return (
    <div className="mt-3">
      <div className="overflow-hidden rounded-xl border border-white/10 bg-[#0a0f18] shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
        <div className="flex items-center justify-between gap-2 border-b border-white/[0.07] bg-white/[0.04] py-1.5 pl-3.5 pr-2">
          <span className="flex min-w-0 items-center gap-2">
            ...
            <span className="truncate font-mono text-[11px] uppercase tracking-wider text-slate-500">
              {label}
            </span>
          </span>
          <button
...
function WorksWhen({ children }: { children: React.ReactNode }) {
  return (
    <p className="mt-3 flex gap-2 rounded-lg border border-emerald-300/20 bg-emerald-300/[0.06] px-3 py-2 text-[12px] leading-relaxed text-emerald-200/90">
      <span className="mt-px grid h-4 w-4 shrink-0 place-items-center rounded-full bg-emerald-300/20 text-[9px] font-bold">
        ✓
      </span>
      <span>
  ...
  return (
    <div className="grid grid-cols-[30px_1fr] gap-x-3.5">
      <div className="flex flex-col items-center">
        <span className="z-10 flex h-[30px] w-[30px] shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-cyan-300 to-emerald-300 text-[13px] font-bold text-teal-950 shadow-md ring-4 ring-[#0d1320]">
          {n}
        </span>
        {!last && <span className="mt-1.5 w-px flex-1 bg-gradient-to-b from-white/15 to-white/[0.04]" />}
      </div>
      <div className={last ? "" : "pb-6"}>
        ...
        <div className="mt-2.5 rounded-xl border border-white/[0.06] bg-white/[0.02] p-3.5">{children}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <>
      <button
        ...
        className="mt-1 inline-flex items-center gap-1.5 rounded-full border border-cyan-300/30 bg-cyan-300/[0.08] px-3.5 py-1.5 text-[12.5px] font-medium text-cyan-200 transition-colors hover:border-cyan-300/60 hover:bg-cyan-300/[0.15]"
      >
      ...
      >
        ...
        <div className="relative shrink-0 border-b border-white/[0.07] px-5 pb-4 pt-5 sm:px-6">
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-3">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-cyan-300/25 to-emerald-300/25 text-xl shadow-inner">
                🚀
              </span>
              <div>
            ...
            <button
              ...
              className="rounded-lg px-2 text-2xl leading-none text-slate-400 transition-colors hover:bg-white/5 hover:text-white"
              aria-label="Close"
            >
              ×
            </button>
          ...
          <div className="mt-3 flex flex-wrap gap-1.5">
            {["⏱ ~15 min total", "📋 copy & paste only", "🧩 4 short steps"].map((t) => (
              <span
                key={t}
                className="rounded-full border border-cyan-300/20 bg-cyan-300/[0.07] px-2.5 py-1 text-[11px] text-cyan-100/80"
              >
                {t}
              </span>
            ))}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 pb-6 pt-4 sm:px-6">
          ...
          <div className="rounded-xl border border-cyan-300/20 bg-cyan-300/[0.06] p-4">
            ...
            <ul className="mt-2 space-y-1.5 text-[13px] leading-relaxed text-slate-300">
              <li>
                ...
                <b>A cheap Linux server (&ldquo;VPS&rdquo;)</b> — Hetzner, DigitalOcean, Vultr… Ubuntu
          ...
          {/* done */}
          <div className="mt-2 rounded-xl border border-emerald-300/25 bg-gradient-to-br from-emerald-300/[0.09] to-cyan-300/[0.06] p-4">
          ...
          <details className="mt-4">
            ...
            <ul className="mt-3 space-y-2">
              {TROUBLES.map((t) => (
                <li
                  key={t.problem}
                  className="rounded-lg border border-white/[0.06] bg-white/[0.02] px-3 py-2.5 text-[12.5px] leading-relaxed"
                >
                  <span className="font-medium text-slate-200">{t.problem}</span>
                  <span className="mt-1 flex gap-1.5 text-slate-400">
